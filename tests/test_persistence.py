...
from yawxt.persistence import PersistMessageHandler, create_all
...
def handler(client, DB_Session, xml_builder):
    ...
    handler = PersistMessageHandler(xml_text, client, 
...
def test_message_save(client, xml_builder, DB_Session, db_session, openid):
    ...
    handler = PersistMessageHandler(xml_text, client, 
        db_session_maker = DB_Session, debug_to_wechat = True)
    handler.reply()
    message = db_session.query(Message).filter_by(msg_id=234567890, from_id=openid).first()
    assert message is not None
    reply_message = db_session.query(Message).filter_by(msg_id=234567890, to_id=openid).first()
...
def test_location_reported(handler, db_session):
    ...
    location = db_session.query(Location).filter_by(openid=handler.openid).order_by(Location.create_time.desc()).first()
