...
import pytest
from yawxt import MessageHandler, check_signature, Message
...
class MessageHandlerTester(MessageHandler):
    ...
    def __init__(self, content, state=None, debug_to_wechat=None):
        self.state = None
        super(MessageHandlerTester, self).__init__(
            content, debug_to_wechat=debug_to_wechat)
...
''', 1234567890123456), 'this is a test'),
    "image": ((
        "image", '''<PicUrl><![CDATA[this is a url]]></PicUrl>
...
def test_event(parametrize, xml_builder):
    ...
    processor = MessageHandlerTester(
        xml_builder(*content), debug_to_wechat=True)
    assert processor.state == expected


@pytest.mark.parametrize(
    "token,nonce,timestamp,signature, time_error",
    [
        (
            "LpxhtFU4GN",
...
def test_reply_image(xml_builder):
    ...
    assert (
        ET.fromstring(message.content)
        .find("Image").find("MediaId").text == "media_id")
