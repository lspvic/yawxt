...
class MessageProcessor(object):
    ...
    def __init__(self, content, account=None,
        ...
        if proc is None:
            self.log("unkown type found: %s" % _id, level=logging.WARNING)
        else:
            proc()
    ...
    def _LOCATION(self):
        ...
        location = Location(lat, lon, precision, self.openid, self.message.create_time)
    ...
    def event_subscribe(self, scene_value, ticket):
        ...
        if scene_value is None:
            ...
            self.debug_to_wechat('您扫码关注了公众号，scene_value: %s, ticket: %s' % (scene_value, ticket))
    ...
    def event_scan(self, scene_value, ticket):
        ...
        '''
        self.reply_debug_text("you scanned the qrcode, scene: %s, ticket: %s" % (scene_value, ticket))
