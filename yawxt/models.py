...
class DictAccess(object):
    ...
    def __eq__(self, value):
        return isinstance(value, self.__class__) and \
            all(getattr(self, k) == getattr(value, k) for k in self.__availabe_keys__)
...
class Message(DictAccess):
    ...
    '''
    __availabe_keys__ = ["to_id", "from_id", "create_time", "msg_id", "msg_type", "content"]
    
    def __init__(self, to_id, from_id, msg_type, content, msg_id = None, create_time=None):
        self.to_id = to_id
    ...
    def from_string(cls, xml_str):
        ...
        if msg_id_node is not None:
            ...
            msg_id = None
        return cls(to_id, from_id, msg_type, ET.tostring(xml).decode(), msg_id, create_time=create_time)
...
<CreateTime>%s</CreateTime>''' % (self.to_id, 
...
class Location(DictAccess):
    ...
    '''
    __availabe_keys__ = ["latitude", "longitude", "precision", "openid", "create_time"]
    
    def __init__(self, latitude, longitude, precision = None, openid=None, create_time=None):
